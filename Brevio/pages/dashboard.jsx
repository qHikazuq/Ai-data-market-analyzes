...
const TICKER_DB = [
  ...
  { ticker: "ZURN", name: "Zurich Insurance Group", type: "Stock" },
  { ticker: "SREN", name: "Swiss Re AG", type: "Stock" },
  ...
  { ticker: "VTI", name: "Vanguard Total Market ETF", type: "ETF" },
    ...
    <div style={{ marginBottom: "16px", background: C.panel, border: "1px solid " + C.borderLight, borderRadius: "10px", overflow: "hidden", animation: "fadeUp 0.35s ease" }}>
      ...
      <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          <button onClick={onOpenDetail} style={{ background: C.accent, border: "none", color: "#0a0c10", borderRadius: "5px", padding: "8px 14px", fontSize: "11px", letterSpacing: "0.1em", cursor: "pointer", fontFamily: "DM Sans, sans-serif", fontWeight: 700, display: "flex", alignItems: "center", gap: "6px" }}>
            <ArrowRight style={{ width: "11px", height: "11px" }} /> Open Full Chart
          </button>
          {!watchlist.includes(result.ticker) && (
            <button onClick={onAdd} style={{ background: "transparent", border: "1px solid " + C.accent + "44", color: C.accent, borderRadius: "5px", padding: "8px 14px", fontSize: "11px", letterSpacing: "0.1em", cursor: "pointer", fontFamily: "DM Sans, sans-serif", display: "flex", alignItems: "center", gap: "6px" }}>
...
export default function App() {
  ...
  useEffect(() => {
    const fn = e => {
      if (wRef.current && !wRef.current.contains(e.target)) setShowSug(false);
      if (addRef.current && !addRef.current.contains(e.target)) setShowAddSug(false);
    };
  ...
  const handleAddInput = val => {
    ...
    const f = TICKER_DB.filter(t => t.ticker.startsWith(val.toUpperCase()) || t.name.toLowerCase().includes(val.toLowerCase())).slice(0, 5);
    setAddSuggestions(f); setShowAddSug(f.length > 0);
  };

  const addStock = (tickerOverride) => {
  ...
  const runAnalysis = async (tickerOverride) => {
  ...
  const STYLES = `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @keyframes livePulse { 0%, 100% { box-shadow: 0 0 4px #3ecf8e; opacity: 1; } 50% { box-shadow: 0 0 10px #3ecf8e; opacity: 0.7; } }
        ...
        <>
          ...
          <div style={{ maxWidth: "760px", margin: "0 auto", padding: "28px 16px 48px" }}>
            ...
            <div style={{ position: "relative" }} ref={addRef}>
              ...
              {showAddSug && (
                <div style={{ position: "absolute", top: "100%", left: 0, right: "58px", background: C.panel, border: "1px solid " + C.accent + "44", borderTop: "1px solid " + C.border, borderRadius: "0 0 6px 6px", overflow: "hidden", zIndex: 50, boxShadow: "0 8px 24px #00000066" }}>
